...
contract MyToken is Context, IERC20, Ownable {
    ...
    address payable public marketingWallet = payable(0x07734164310508D1fA9f4341268C5A4A07dE94Aa);
    uint256 private _tTotal = 1_000_000_000 ether;  // 1bil default Total Supply
    string public constant name = "My Token";  // Token Name
    string public constant symbol = "MYTK";  // Token Symbol
    uint256 public initialBuyFee = 15;  // Initial Buy Fee (up to 15%)
    uint256 public initialSellFee = 15;  // Initial Sell Fee (up to 15%)
    uint256 public finalBuyFee = 2;  // 2% Final Fee (during renounce)
    uint256 public finalSellFee = 2;  // 2% Final Fee (during renounce)
    uint256 public maxTxAmount = 100_000 ether;  // 1% default
    uint256 public maxWalletAmount = 100_000 ether;  // 1% default
    uint256 public minimumTokensBeforeSwap = 100_000 ether;  // 0.01% default (Sets a threshold of tokens that must be collected in the contract before a swap operation is triggered)
